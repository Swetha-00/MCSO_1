...
%CSO Parameter Setting Start    
 ...
 GlobalBest_Cost=inf;
  for i=1:pop_size
     ...
     val(i,1)=fitness(i);
      % Update Global Best
    if val(i,1)<GlobalBest_Cost
        
        bsf_solution=pop(i,:);
        GlobalBest_Cost=val(i,1);
    end
  end
 %END CSO Parameter Setting
    %% main loop
    index = 1;
    while nfes < max_nfes
        %*****************************CSO STARTING************************
%     newpop=pop;    
%    rlist = randperm(pop_size); 
%    rpairs = [rlist(1:ceil(pop_size/2)); rlist(floor(pop_size/2) + 1:pop_size)]';
%    
%    mask = (val(rpairs(:,1)) > val(rpairs(:,2)));
%    losers = mask.*rpairs(:,1) + ~mask.*rpairs(:,2);  
%    winners = ~mask.*rpairs(:,1) + mask.*rpairs(:,2);       
%    
% 
% 
%    randco1 = rand(ceil(pop_size/2),problem_size);
%    randco2 = rand(ceil(pop_size/2),problem_size);
%    randco3 = rand(ceil(pop_size/2),problem_size);
% 
%    center = ones(ceil(pop_size/2),1)*mean(pop);
%    
%    vel(losers,:)=randco1.*vel(losers,:) ...,
%                     + randco2.*(pop(winners,:) - pop(losers,:)) ...,
%                     + phi*randco3.*(center - pop(losers,:));
%   
% 
% 
%    
%     %newpop(1:NP,:)=pop(FVr_a1,:)+ 0.7*(pop(FVr_a2,:)-pop(FVr_a3,:));
%     newpop(losers,:) = newpop(losers,:) + vel(losers,:);
%----------------------------CSO ENDS-----------------------------------
 %*****************************MCSO STARTING************************   
   newpop=pop;
   rlist = randperm(pop_size); 
   rpairs = [rlist(1:ceil(pop_size/3)); rlist(floor(pop_size/3)+1:ceil(2*pop_size/3)-1);rlist(ceil(2*pop_size/3):pop_size)]';
   
   mask1 = (val(rpairs(:,1)) > val(rpairs(:,2)));
   losersa = mask1.*rpairs(:,1) + ~mask1.*rpairs(:,2);  
   winnersa = ~mask1.*rpairs(:,1) + mask1.*rpairs(:,2);       
   
   mask2 = (val(rpairs(:,1)) > val(rpairs(:,3)));
   losersb = mask2.*winnersa(:,1) + ~mask2.*rpairs(:,3);  
   winners = ~mask2.*winnersa(:,1) + mask2.*rpairs(:,3); 
   
   mask3 = (val(losersa(:,1)) > val(losersb(:,1)));
   losers2 = mask3.*losersa(:,1) + ~mask3.*losersb(:,1);  
   losers1 = ~mask3.*losersa(:,1) + mask3.*losersb(:,1);

   randco1 = rand(ceil(pop_size/3),problem_size);
   randco2 = rand(ceil(pop_size/3),problem_size);
   randco3 = rand(ceil(pop_size/3),problem_size);
   randco4 = rand(ceil(pop_size/3),problem_size);
   randco5 = rand(ceil(pop_size/3),problem_size);
   randco6 = rand(ceil(pop_size/3),problem_size);
   center1 = ones(ceil(pop_size/3),1)*mean(pop);
   center2 = ones(ceil(pop_size/3),1)*mean(pop(winners,:));
   vel(losers1,:)=randco1.*vel(losers1,:) ...,
                    + randco2.*(pop(winners,:) - pop(losers1,:)) ...,
                    + phi*randco3.*(center1 - pop(losers1,:));
  
   vel(losers2,:)=randco4.*vel(losers2,:) ...,
                    + randco5.*(pop(winners,:) - pop(losers2,:)) ...,
                    + phi*randco6.*(center2 - pop(losers2,:));

   
    %newpop(1:NP,:)=pop(FVr_a1,:)+ 0.7*(pop(FVr_a2,:)-pop(FVr_a3,:));
    newpop(losers1,:) = newpop(losers1,:) + vel(losers1,:);
    newpop(losers2,:) = newpop(losers2,:) + vel(losers2,:);
    %...................MCSO ENDS..................
